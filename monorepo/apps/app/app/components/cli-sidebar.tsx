...
export function CLISidebar({ onCommandSelect, selectedCommand, className }: CLISidebarProps) {
  ...
  const categories = [
    { id: "contract", label: "Smart Contracts", color: "bg-blue-500" },
    { id: "deployment", label: "Deployment", color: "bg-green-500" },
    { id: "flow", label: "Flow CLI", color: "bg-purple-500" },
    { id: "documentation", label: "Documentation", color: "bg-yellow-500" },
    { id: "system", label: "System", color: "bg-gray-500" },
    { id: "chat", label: "Chat", color: "bg-pink-500" },
  ] as const
      ...
      <ScrollArea className="flex-1">
        <div className="p-2 space-y-1">
          {categories.map(category => {
            const commands = getCommandsByCategory(category.id as any)
      ...
      <div className="p-4 border-t">
        <div className="text-xs text-muted-foreground">
          ...
          <div className="flex items-center justify-between">
            <span>Commands</span>
            <span className="font-mono">{CLI_COMMANDS.length}</span>
          </div>
        </div>
      </div>
    </div>
