...
function StreamingWidget({ title, icon, children, className = "", isLive = false }: WidgetProps) {
  return (
    <motion.div
      ...
      className={`relative bg-card/80 rounded-2xl border border-border/70 p-6 shadow-md overflow-hidden ${className}`}
    >
      {/* Live indicator */}
      {isLive && (
        <motion.div
          initial={{ scale: 0 }}
          animate={{ scale: 1 }}
          className="absolute top-4 right-4 flex items-center space-x-2 px-2 py-1 bg-primary/10 border border-primary/20 rounded-full"
        >
            ...
            {activeGenerations.map((generation) => (
              ...
              >
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-xs text-primary font-medium">{Math.round(generation.progress)}%</span>
                </div>
...
function SystemStatusWidget() {
  ...
  const getStatusBg = (value: number, thresholds: [number, number]) => {
    if (value < thresholds[0]) return "from-primary/20 to-primary/10"
    if (value < thresholds[1]) return "from-muted/40 to-muted/20"
  ...
  return (
    <StreamingWidget
      title="System Status"
      icon={isOnline ? <Wifi className="h-5 w-5 text-primary" /> : <WifiOff className="h-5 w-5 text-destructive" />}
      isLive={isOnline}
    >
      <div className="space-y-4">
        {/* Connection Status */}
        <div className={`flex items-center justify-between p-3 rounded-lg bg-gradient-to-r ${
          isOnline ? 'from-primary/10 to-primary/5 border-primary/20' : 'from-destructive/10 to-destructive/5 border-destructive/20'
        } border`}>
          <div className="flex items-center space-x-2">
            <div className={`w-2 h-2 rounded-full ${isOnline ? 'bg-primary animate-pulse' : 'bg-destructive'}`} />
            <span className="text-sm font-medium">{isOnline ? 'Online' : 'Offline'}</span>
          </div>
          <span className={`text-xs ${isOnline ? 'text-primary' : 'text-destructive'}`}>
