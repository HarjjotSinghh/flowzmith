"""
{{CONTRACT_NAME}} MCP Client - Test client for {{CONTRACT_NAME}} MCP Server

This client provides a simple interface to test the {{CONTRACT_NAME}} MCP server tools.
...
class ContractMCPClient:
    """Client for interacting with {{CONTRACT_NAME}} MCP Server."""
    ...
    async def connect(self):
        ...
        print(f"Available tools for {{CONTRACT_NAME}}:")
    ...
    async def view_contract_info(self, network: str = "{{NETWORK}}") -> Dict[str, Any]:
...
async def main():
    """Example usage of the {{CONTRACT_NAME}} MCP Client."""
    import sys
    ...
    try:
        print(f"Connecting to {{CONTRACT_NAME}} MCP Server...")
        await client.connect()
        
        print("\n" + "="*50)
        print("{{CONTRACT_NAME}} MCP Client Test Suite")
        print("="*50)
